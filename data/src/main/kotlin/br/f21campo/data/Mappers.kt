...
fun Occupation.toEntity() = OccupationEntity(id.value, projectId.value, stationId.value, referencePointId?.value, plannedDurationSeconds, state.name, plannedStart?.toEpochMilli(), confirmedStart?.toEpochMilli(), confirmedStop?.toEpochMilli(), equipment?.receiver?.model, equipment?.antenna?.model, equipment?.receiver?.manufacturer, equipment?.antenna?.manufacturer, equipment?.receiver?.serialNumber, equipment?.antenna?.serialNumber, hasBeforeHeight, beforeHeightMeters)
