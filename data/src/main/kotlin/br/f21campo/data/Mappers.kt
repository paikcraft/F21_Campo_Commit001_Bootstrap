...
fun Occupation.toEntity() = OccupationEntity(id.value, projectId.value, stationId.value, state.name, plannedStart?.toEpochMilli(), confirmedStart?.toEpochMilli(), confirmedStop?.toEpochMilli(), equipment?.receiver?.model, equipment?.antenna?.model, hasBeforeHeight)
fun OccupationEntity.toDomain() = Occupation(EntityId(id), EntityId(projectId), EntityId(stationId), plannedStartEpochMillis?.let(Instant::ofEpochMilli), confirmedStartEpochMillis?.let(Instant::ofEpochMilli), confirmedStopEpochMillis?.let(Instant::ofEpochMilli), OccupationState.valueOf(state), hasBeforeHeight = hasBeforeHeight)
