...
fun Occupation.toEntity() = OccupationEntity(id.value, projectId.value, stationId.value, referencePointId?.value, plannedDurationSeconds, state.name, plannedStart?.toEpochMilli(), confirmedStart?.toEpochMilli(), confirmedStop?.toEpochMilli(), equipment?.receiver?.model, equipment?.antenna?.model, equipment?.receiver?.serialNumber, equipment?.antenna?.serialNumber, hasBeforeHeight, beforeHeightMeters)
fun OccupationEntity.toDomain() = Occupation(EntityId(id), EntityId(projectId), EntityId(stationId), referencePointId?.let(::EntityId), plannedDurationSeconds, plannedStartEpochMillis?.let(Instant::ofEpochMilli), confirmedStartEpochMillis?.let(Instant::ofEpochMilli), confirmedStopEpochMillis?.let(Instant::ofEpochMilli), OccupationState.valueOf(state), equipment = if (receiverModel != null || antennaModel != null || receiverSerial != null || antennaSerial != null) EquipmentSnapshot(Receiver(EntityId.new(), model = receiverModel, serialNumber = receiverSerial), Antenna(EntityId.new(), model = antennaModel, serialNumber = antennaSerial)) else null, hasBeforeHeight = hasBeforeHeight, beforeHeightMeters = beforeHeightMeters)
